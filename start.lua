...
print("::Black::")
local RunBot = io.open("MekoTeam", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/MekoTeam
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr MekoTeam.lua
wget "https://raw.githubusercontent.com/LARICATEAM/MekoTeam/master/MekoTeam.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./MekoTeam.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S MekoTeam -X kill
screen -S MekoTeam ./MekoTeam
done
]])
RunTs:close()
end
